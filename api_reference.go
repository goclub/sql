...
type APIDatabase interface {
	...
	// 硬删除（不可恢复）
	HardDelete(ctx context.Context, qb QB, checkSQL ...string) (result sql.Result, err error)
	// 基于 Model 硬删除（不可恢复）
	...
	// 执行SQL
	Exec(ctx context.Context, qb QB, statement Statement) (result sql.Result, err error)
	// 开启事务

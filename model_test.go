...
type User struct {
	...
	TableUser
	// 每个 Model 都应该具有生命周期触发函数 BeforeInsert() AfterInsert() BeforeUpdate() AfterUpdate() 方法
...
// 因为 user 表的 id 字段是 uuid，所以在 User 的 BeforeInsert 生命周期去创建 id
...
func (UserWithAddress) SoftDeleteWhere() sq.Raw {
	return sq.Raw{"`user`.`deleted_at` IS NULL AND `user_address`.`deleted_at` IS NULL", nil}
}
